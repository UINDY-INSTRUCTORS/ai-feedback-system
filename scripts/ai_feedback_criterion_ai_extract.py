...
This version uses AI to extract relevant sections instead of deterministic keyword matching.
"""
...
USE_AI_EXTRACTION = os.environ.get('USE_AI_EXTRACTION', 'true').lower() == 'true'

if USE_AI_EXTRACTION:
    from ai_section_extractor import extract_sections_for_criterion_ai
    print("Using AI-based section extraction")
else:
    from section_extractor import extract_sections_for_criterion
    print("Using deterministic keyword-based extraction")
...
def build_criterion_prompt(report: dict, criterion: dict, guidance_excerpt: str) -> str:
    ...
    # Extract relevant sections - using AI or deterministic method
    if USE_AI_EXTRACTION:
        print(f"   Using AI to extract relevant sections...")
...
def main():
    ...
    print("\n" + "="*60)
    print("AI Feedback System (with AI-based section extraction)")
    print("="*60)
    ...
    print(f"\nUsing model: {model}")
    print(f"Extraction method: {'AI-based' if USE_AI_EXTRACTION else 'Deterministic keyword-based'}")
